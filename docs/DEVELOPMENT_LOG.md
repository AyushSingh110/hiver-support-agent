...
## Phases 6c–6g — Generation, evaluation, escalation, judge, human ratings, packaging

**Dates:** 2026-09-16 to 2026-09-17
**Status:** Complete for submission. Condensed here; the reasoning is in D43–D49 and the
results are in `REPORT.md`.

### 6c — Grounded generation
- **Design:** `llama3.1:8b` through Ollama with `urllib`, temperature 0, seed 42, JSON
  output, and a disk cache.
- **Checks and prompt:**
  - deterministic checks G1–G6;
  - G7 (copying ≥ 0.90 similarity) and prompt p2, added after a smoke test showed a reply
    copied word for word.
- **Parser:** a smoke test showed ranks returned as strings, so the parser now converts
  digit strings and records that it did (`evidence_used_coerced`).
- **Golden run:**
  - 248 rows;
  - **49 failed on the first attempt**: 47 out-of-memory errors on the host, plus one
    HTTP 500 and one empty model response;
  - each failed row was retried once with identical settings, and all 49 succeeded;
  - the first pass and the retry were merged by hand; `src/evaluate.py` verifies the merge.
- **Determinism probe:** 4 of 5 rows regenerated byte for byte. b01_0001, the first request
  after the model loaded, did not.

### 6d — Escalation
- **Rules:** seven ordered rules in `src/escalate.py`, with an explicit human-review policy
  (D44) and a separate "needs details" code (D45).
- **Thresholds:** `src/calibrate_escalation.py` set them as 20th percentiles on a golden-free
  pool of 2,000 conversations (D43). Two runs gave byte-identical output.

### 6e — Deterministic evaluation and the LLM judge
- **Evaluation:**
  - `src/evaluate.py` reproduces predictions, evidence, flags and baselines exactly before
    scoring;
  - golden labels are read only with `--confirm-golden-run`;
  - a second run of the golden evaluation gave byte-identical rows.
- **Judge:** `src/judge.py` went j1 → j2 → j3 on qwen2.5:7b, then j3 on qwen3:8b. **Every
  configuration failed its pre-declared non-golden validation** (D46). No judge was run on
  golden rows.
- **Cache hygiene:** the judge's sanity and probe caches were isolated after finding that
  `llm.complete` writes to the cache even when it is bypassed.

### 6f — Human reply-quality ratings
- **Batch:** 40 items × 3 systems, blinded; seeds 46 and 47 (D47).
- **Round 1:**
  - returned 45 minutes after the batch was built;
  - its scores were saved as `N.0` and re-saved as integers with approval;
  - it was validated without viewing any values.
- **Analysis code:** written after round 1 came back but without reading it, and tested
  only on synthetic data.
- **Retest:** the batch was built (36 rows), but the 72-hour gap could not be met before
  submission. **Round 1 was analysed descriptively and the retest closed as not completed**
  (D48).
- **Disclosure:** the rater is the developer, and the ratings were AI-assisted.

### 6g — Packaging
- **Artifacts:** the recorded 6C generations are tracked in `artifacts/phase6/`.
- **Byte-exact storage:** `.gitattributes` stores data files unchanged. `core.autocrlf` had
  been changing the golden and human-evaluation CSVs in the repository.
- **One command:** `python -m src.reproduce_headline` reproduces the headline evaluation
  from the raw CSV without an LLM (D49). Its timing was measured on a clean copy of the
  repository (README).
