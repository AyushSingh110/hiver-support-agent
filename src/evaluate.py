...
def run_golden_evaluation(*, confirm: bool = False) -> dict:
    ...
    for subset, subset_rows in _subsets(rows).items():
        ...
        stats["generation"][subset] = {
            **generation_summary([r["system"]["generation"] for r in subset_rows]),
            "parse_failure_ids": [r["annotation_id"] for r in subset_rows
                                  if not r["system"]["generation"]["parsed"]],
            "claim_flag_rows": [
                {"annotation_id": r["annotation_id"],
                 "codes": [c for c in r["system"]["generation"]["grounding_flag_codes"]
                           if c != "G1_empty_reply"]}
                for r in subset_rows
                if any(c != "G1_empty_reply" for c in r["system"]["generation"]["grounding_flag_codes"])
            ],
        }
...
def render_report(stats: dict) -> str:
    ...
    subsets = list(stats["classifier"])
    prov = stats["provenance"]
    frozen = prov["frozen_configuration"]
    lines = [
        "# Phase 6E — Deterministic evaluation on the golden set",
        "",
        "Generated by `src/evaluate.py --confirm-golden-run`. No LLM was called. An LLM judge was "
        "built and tested experimentally, but it failed its pre-declared validation checks "
        "(docs/DECISION_LOG.md D46). It was not run on the golden set and is not an evaluation "
        "metric.",
        "",
        "## Methodology",
        "",
        "- **Evaluation set.** 248 golden rows: 148 pilot rows (b01) revised under taxonomy v2, and "
        "100 fresh rows (b02). **b02 is the independent fresh subset.** The 248 rows are not an "
        "independently sampled 248-example human set. Every table reports both.",
        "- **Isolation.** Before any metric, the harness re-checked that the retrieval corpus and weak "
        "training set contain no golden conversation, customer, exact text or near-duplicate, that "
        "threshold calibration touched no golden data, and that the configuration matches the frozen run.",
        "- **Run of record.** Generation is scored from the recorded Phase 6C outputs, not regenerated. "
        "The harness reproduced the recorded classifier predictions, retrieved evidence, G1–G7 flags and "
        "both baselines exactly before scoring; any mismatch would have stopped the run.",
        "- **Classifier.** Majority class and the frozen TF-IDF + logistic regression, both fitted on "
        "golden-free weak labels only. 95% intervals: percentile bootstrap, "
        f"{prov['bootstrap']['resamples']} resamples, seed {prov['bootstrap']['seed']}.",
        "- **Retrieval.** TF-IDF k=5, and the frozen random baseline (per-query seed 42 + query index) "
        "under identical exclusions. **Retrieved evidence has no human gold labels**, so both intent "
        "measures compare against rule-based weak labels and neither is retrieval accuracy.",
        "- **Escalation.** The frozen esc-v1 rules and D43 thresholds, run on golden rows for the first "
        "time. There are no escalation outcome labels; every escalation figure is descriptive.",
        "- **Gold labels** are held in a separate `gold` field of each row record and are read only for "
        "scoring and breakdowns.",
        "",
        "## Classifier",
        "",
    ...
    for subset in subsets:
        for name, result in stats["classifier"][subset].items():
            m, b = result["metrics"], result["bootstrap"]
            lines.append(f"| {name} | {subset} | {m['accuracy']} {b['accuracy']} | "
                         f"{m['macro_f1']} {b['macro_f1']} | {m['weighted_f1']} |")

    outside = {s: stats["classifier"][s]["baseline_1_tfidf_logreg"]["secondary_predictable_intents"]
               ["rows_outside_view"] for s in subsets}
    lines += ["", "Gold labels outside the classifier's predictable label space "
              "(`non_support_commentary`, `OTHER`, `UNCLEAR`): "
              + ", ".join(f"{n} rows in {s}" for s, n in outside.items())
              + ". These rows stay in the headline results above; the classifier has no training "
              "label for them.", "",
              "**Secondary view** — only rows whose gold label is one of the 10 predictable intents, "
              "scored over those 10 labels. It does not replace the headline result.", "",
    ...
    lines += ["", "**Per-intent, TF-IDF + logistic regression**", "",
              "| Intent | " + " | ".join(f"{s} support / P / R / F1" for s in subsets) + " |",
              "| --- | " + " | ".join("---" for _ in subsets) + " |"]
    for label in ALL_LABELS:
        cells = []
        for subset in subsets:
            x = stats["classifier"][subset]["baseline_1_tfidf_logreg"]["metrics"]["per_intent"][label]
            cells.append(f"{x['support']} / {x['precision']} / {x['recall']} / {x['f1']}")
        lines.append(f"| {label} | " + " | ".join(cells) + " |")
    lines += ["", "Full confusion matrices are in `phase6_evaluation_stats.json`."]

    lines += ["", "## Retrieval (k=5)", "",
              "Retrieved evidence carries only rule-based weak labels, never human labels. "
              "**Neither measure below is retrieval accuracy.**", "",
              "| System | Subset | Predicted intent vs weak label | Gold intent vs weak label | "
              "Queries with any gold match | Weak-labelled items | Coverage |",
              "| --- | --- | --- | --- | --- | --- | --- |"]
    for subset in subsets:
        for name, result in stats["retrieval"][subset].items():
            a = result["predicted_intent_vs_weak_label_consistency"]
            g = result["gold_intent_vs_weak_label_agreement"]
            lines.append(
                f"| {name} | {subset} | {a['agreement_pct_of_weakly_labelled_items']}% "
                f"({a['matching_items']}/{a['evidence_with_weak_label']}) | "
                f"{g['agreement_pct_of_weakly_labelled_items']}% "
                f"({g['matching_items']}/{g['evidence_with_weak_label']}) | "
                f"{pct(g['queries_with_any_match'])} | {g['evidence_with_weak_label']}/{g['evidence_items']} | "
                f"{g['weak_label_coverage_pct']}% |")
    lines.append("")
    for subset in subsets:
        sim = stats["retrieval"][subset]["tfidf"]["similarity"]
        t1, tk = sim["top1_similarity"], sim["all_topk_similarity"]
        lines.append(f"- **{subset}, TF-IDF:** {sim['queries'] - sim['queries_without_evidence']}/"
                     f"{sim['queries']} queries returned evidence. Top-1 similarity median "
                     f"{t1['median']} (min {t1['min']}, max {t1['max']}); all top-k median {tk['median']}. "
                     "Similarity measures lexical overlap, not evidence quality.")

    k = stats["retrieval"]["development_pool_k_check"]
    lines += ["", "### k-selection disclosure", "",
              "**k=5 was selected in Phase 6B by a pre-declared rule that was applied to golden "
              "results** (D42). That is a model choice which consulted the evaluation set. As a "
              "sensitivity check, the same rule was re-applied on the golden-free Phase 6D development "
              f"pool ({k['queries']} weakly-labelled queries, weak vs weak):", "",
              f"- k=3: {k['k3']['agreement_pct_of_weakly_labelled_items']}% consistency",
              f"- k=5: {k['k5']['agreement_pct_of_weakly_labelled_items']}% consistency",
              f"- rule ({k['rule']}) selects **k={k['rule_selects']}**",
              "",
              f"**The evaluated configuration keeps k={k['frozen_k']} regardless of this result.**"]

    lines += ["", "## Generation checks (recorded Phase 6C outputs)", "",
              "G1–G7 are pattern detectors. Passing them is not reply correctness, helpfulness or "
              "resolution.", "",
    ...
    for subset in subsets:
        ...
        for name, result in entries:
            ...
            lines.append(f"| {name} | {subset} | {pct(result['all_checks_passed'])} | "
                         + " | ".join(flags) + " |")
    lines += ["", f"**Baseline A.** {BASELINE_A_CAVEAT}", "",
              "**Baseline B** is one fixed sentence with no URL, number or claim, so passing every "
              "check is also a property of its definition.", ""]
    for subset in subsets:
        g = stats["generation"][subset]
        flagged = ", ".join(f"{r['annotation_id']} ({', '.join(r['codes'])})"
                            for r in g["claim_flag_rows"]) or "none"
        lines += [
            f"**LLM, {subset}:**",
            f"- parsed {pct(g['parsed'])}; parse failures {pct(g['parse_failures'])} "
            f"({g['parse_failure_kinds']}): {', '.join(g['parse_failure_ids']) or 'none'}",
            f"- rows with G2–G7 flags: {flagged}",
            f"- needs_more_information true {pct(g['needs_more_information_true'])}, "
            f"false {pct(g['needs_more_information_false'])}",
            f"- citations within range {pct(g['citation_valid'])}; citing no evidence "
            f"{pct(g['citing_no_evidence'])}; digit-string ranks coerced {pct(g['evidence_used_coerced'])}",
            f"- max copy similarity to any evidence reply: median {g['max_copy_similarity']['median']}, "
            f"max {g['max_copy_similarity']['max']}",
            f"- latency (s): median {g['latency_seconds']['median']}, mean "
            f"{g['latency_seconds']['mean']}, max {g['latency_seconds']['max']}; missing {g['latency_missing']}",
            f"- attempts {g['attempts']} (attempt 2 = one infrastructure retry in Phase 6C)",
            "",
        ]

    lines += ["## Escalation (descriptive)", "",
              "No human label says whether any message should have been escalated. These figures "
              "describe what the frozen rules did; they are not accuracy, precision, recall or "
              "correctness.", ""]
    for subset in subsets:
        e = stats["escalation"][subset]
        lines += [f"### {subset}", "",
                  f"Escalate {pct(e['escalate'])}; auto-handle {pct(e['auto_handle'])}; "
                  f"`awaiting_customer_details` attached to "
                  f"{e['informational_code_counts'].get('awaiting_customer_details', 0)} rows.", "",
                  "| Reason code | Rows |", "| --- | --- |"]
        lines += [f"| {code} | {pct(v)} |" for code, v in e["reason_code_counts"].items()]
        lines += ["", "| Reason-code combination | Rows |", "| --- | --- |"]
        lines += [f"| {combo} | {count} |" for combo, count in e["reason_code_patterns"].items()]
        lines += ["", "| Gold intent | Escalated |", "| --- | --- |"]
        lines += [f"| {intent} | {pct(v)} |" for intent, v in e["escalation_rate_by_gold_intent"].items()]
        lines += ["",
                  f"- weak-grounding rule fired: {pct(e['weak_grounding_rule_fired'])}",
                  f"- predicted praise with the weak-grounding rule fired: "
                  f"{pct(e['predicted_praise_weak_grounding'])}",
                  f"- escalated when the classifier was correct: "
                  f"{pct(e['escalation_rate_when_classifier_correct'])}; when wrong: "
                  f"{pct(e['escalation_rate_when_classifier_wrong'])}",
                  f"- escalated when the gold label is outside the predictable label space: "
                  f"{pct(e['escalation_rate_gold_label_not_predictable'])}",
                  ""]

    h = stats["human_agreement"]
    lines += ["## Human annotation evidence", "",
              f"**{h['measurement']}.** n = {h['n']}, gap {h['gap']}. Raw agreement "
              f"{h['raw_agreement_pct']}%, Cohen's κ = {h['cohens_kappa']} (95% bootstrap CI "
              f"{h['kappa_ci_95']}, {h['bootstrap_resamples']} resamples, seed {h['bootstrap_seed']}). "
              "One annotator produced every golden label; no second annotator exists. The retest "
              "labels are not used as evaluation labels.", "",
              "## Coverage", ""]
    lines += [f"- {key}: {value}" for key, value in stats["coverage"].items()]

    lines += ["", "## Limitations", "",
              "- No metric here measures reply correctness, helpfulness or whether an issue was resolved.",
              "- Escalation has no outcome labels; its thresholds are traffic sizes (D43), not tuned values.",
              "- Retrieval intent measures depend on weak labels that cover roughly 38–41% of evidence "
              "items and only 10 intents.",
              "- The classifier cannot predict 3 of the 13 taxonomy labels.",
              "- k=5 was chosen in Phase 6B after consulting golden results (disclosed above).",
              "- All golden labels come from one annotator; agreement evidence is intra-annotator only.",
              "- b02 has 100 rows, so its intervals are wide and per-intent figures rest on very small supports.",
              "- 49 generations needed one infrastructure retry in Phase 6C, and one of five probed rows "
              "(b01_0001) did not reproduce byte for byte; the recorded outputs are the run of record.",
              "", "## Reproducibility", "",
              f"- Frozen configuration: `{json.dumps(frozen)}`",
              f"- Prompt template SHA-256: `{prov['prompt_template_sha256']}`",
              f"- Random retrieval seed: {prov['random_retrieval_seed']}",
              f"- git HEAD: `{prov['git_head']}`",
              f"- Versions: `{json.dumps(prov['versions'])}`",
              "- File SHA-256:"]
    lines += [f"  - {name}: `{digest}`" for name, digest in prov["file_sha256"].items()]
    return "\n".join(lines) + "\n"
