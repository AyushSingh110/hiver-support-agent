...
def render_report(stats: dict, random_table, targeted_table, pairs, triggers, review, source: str) -> str:
    ...
    lines = [
        ...
        "",
        f"Generated by `src/analyse_annotations.py` from `{source}` against taxonomy "
        ...
        "rows that produced them is circular: the numbers below describe what the "
        "annotator decided, they do **not** show that the taxonomy is correct or that a "
        "classifier would reproduce it. The golden set is the first honest test. "
        "See `docs/DECISION_LOG.md` D38.",
        "",
...
def main() -> int:
    ...
    stats = {
        ...
        "output_is_validation": False,
        "output_nature": (
            "retrospective re-description of an already-labelled batch; the v2 "
            "boundaries were written after reading these rows, so these numbers do "
            "not validate the taxonomy"
        ),
        "validation": checks,
